...
type Props = {
  params: { townSlug: string; serviceSlug: string };
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${service.title} in ${location.name} | Fast Cyber Cafe Processing`,
    description: `Apply for ${service.title} in ${location.name} today. Fast, secure eCitizen processing at our ${location.name} digital bridge. No queues, instant support.`,
  };
}
    ...
    <div className="bg-white min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 text-center lg:text-left">
              <nav className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-8 justify-center lg:justify-start">
                  ...
                  <span className="text-emerald-400">Cyber Bridge</span>
              </nav>
              <div className="flex items-center gap-6 mb-12 justify-center lg:justify-start">
                  ...
                  <div>
                      ...
                      <p className="text-white font-black text-lg uppercase tracking-tighter">{townSlug} Node 0{Math.floor(Math.random() * 9) + 1}</p>
                  </div>
              </div>
              ...
              <p className="text-xl text-slate-400 max-w-xl leading-relaxed mb-10 font-medium italic">
                Official application gateway for {service.title} specialists for citizens in {location.name}.
              </p>
            ...
            <div className="lg:w-1/2 w-full">
                <div className="p-10 glass rounded-[3.5rem] border border-white/10 shadow-3xl">
                    <div className="mb-10 text-center">
                        ...
                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-2">{location.name} Central Terminal</p>
                    </div>
      ...
      <section className="py-32 bg-slate-50">
          <div className="max-w-4xl mx-auto px-6 text-center">
              <h2 className="text-3xl font-black text-slate-900 mb-12 tracking-tight uppercase">Support in {location.name}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                  ...
                  <div className="bg-white p-10 rounded-3xl shadow-sm border border-slate-100">
                      ...
                      <p className="text-slate-500 text-sm leading-relaxed font-medium font-medium">As part of our commitment to the residents of {location.county} County, all biometric data and personal identifiers are purged automatically 48 hours after application completion.</p>
